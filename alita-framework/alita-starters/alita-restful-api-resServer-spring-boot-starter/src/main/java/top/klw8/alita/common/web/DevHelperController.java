...
public class DevHelperController {
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "isAdd2SuperAdmin", value = "是否自动添加到超级管理员角色和帐号", dataType = "boolean", paramType = "query", example = "true", defaultValue = "true", required = true)})
    ...
    public Mono<JsonResult> registeAllAuthority(boolean isAdd2SuperAdmin) {
        ...
        for (Entry<RequestMappingInfo, HandlerMethod> methodEntry : methodMap.entrySet()) {
            ...
            if (method.isAnnotationPresent(AuthorityRegister.class)) {
                ...
                if (controllerClass.isAnnotationPresent(AuthorityCatlogRegister.class)) {
                    AuthorityCatlogRegister catlogRegister = controllerClass
                            .getAnnotation(AuthorityCatlogRegister.class);
                    if (StringUtils.isBlank(catlogRegister.name())) {
                        return Mono.just(JsonResult
                                .failed(controllerClass.getName() + "的 AuthorityCatlogRegister 中没有设置catlog名称,注册失败"));
                    }
                ...
                au.setRemark(moduleName + (StringUtils.isBlank(register.authorityRemark()) ? register.authorityName()
                        : register.authorityRemark()));
                
                List<AlitaAuthoritysResource> authoritysResourceList = new LinkedList<>();
                // 解析资源权限枚举源
                if (register.authoritysResourceSourceEnum() != IAuthoritysResourceSourceItem.class) {
                    if (Enum.class.isAssignableFrom(register.authoritysResourceSourceEnum())) {
                        Object[] enumConstants = register.authoritysResourceSourceEnum().getEnumConstants();
                        for (Object obj : enumConstants) {
                            ...
                            sds.setResType(enumItem.getResType());
                            authoritysResourceList.add(sds);
                        }
                    } else {
                        log.warn("【{}】中配制的资源权限源【{}#authoritysResourceSourceEnum()】仅支持枚举类型,将忽略...", authorityAction,
                                AuthorityRegister.class.getName());
                    }
                }
                
                // 解析资源权限源,有就入库,没有就继续
                if (register.authoritysResourceSource() != IAuthoritysResourceSource.class) {
                    // 有资源权限源
                    IAuthoritysResourceSource staticSource = applicationContext.getBean(register.authoritysResourceSource());
                    Assert.notNull(staticSource, "【" + authorityAction + "】没有找到资源权限静态数据源,数据源需要放入spring容器中,请检查");
                    List<IAuthoritysResourceSourceItem> itemList = staticSource.getAuthoritysResourceSourceList();
                    if (CollectionUtils.isEmpty(itemList)) {
                        log.warn("【" + authorityAction + "】中配制的资源权限静态数据源返回结果为空,请检查!");
                    } else {
                        for (IAuthoritysResourceSourceItem item : itemList) {
                            AlitaAuthoritysResource sds = new AlitaAuthoritysResource();
                            sds.setResource(item.getResource());
                            sds.setRemark(item.getRemark());
                            sds.setResType(item.getResType());
                            authoritysResourceList.add(sds);
                        }
                    }
                }
                
                if (!authoritysResourceList.isEmpty()) {
                    au.setAuthoritysResourceList(authoritysResourceList);
                }
        ...
        // 处理全局资源权限
        List<AlitaAuthoritysResource> publicAuthoritysResourceList = new ArrayList<>(16);
        Map<String, Object> publicAuthoritysResources = applicationContext
                .getBeansWithAnnotation(PublicAuthoritysResourceRegister.class);
        if (null != publicAuthoritysResources && !publicAuthoritysResources.isEmpty()) {
            publicAuthoritysResources.forEach((key, value) -> {
                if (!IAuthoritysResourceSource.class.isAssignableFrom(value.getClass())) {
                    log.error("【{}】使用了 @PublicAuthoritysResourceRegister 注解,但没实现 IAuthoritysResourceSource 接口",
                            value.getClass().getName());
                } else {
                    Class<?> vClass = value.getClass();
                    PublicAuthoritysResourceRegister ann = vClass.getAnnotation(PublicAuthoritysResourceRegister.class);
                    IAuthoritysResourceSource dsSource = (IAuthoritysResourceSource) value;
                    List<IAuthoritysResourceSourceItem> dsSourceList = dsSource.getAuthoritysResourceSourceList();
                    if (CollectionUtils.isEmpty(dsSourceList)) {
                        log.warn("【{}】中返回的全局资源权限数据为空,将没有任何全局资源权限入库", value.getClass().getName());
                    } else {
                        // 静态数据源,入库
                        for (IAuthoritysResourceSourceItem item : dsSourceList) {
                            ...
                            sds.setRemark(item.getRemark());
                            sds.setResType(item.getResType());
                            publicAuthoritysResourceList.add(sds);
                        }
                    }
                }
            });
        ...
        return Mono.fromFuture(devHelperProvider
                .batchAddAuthoritysAndCatlogs(new ArrayList<>(tempMap.values()), publicAuthoritysResourceList,
                        isAdd2SuperAdmin, currectApp.getAppEntity()));
    }
    ...
    @ResponseBody
    public Mono<String> statusCodeInfo()
        ...
        try {
            ...
            if (StringUtils.isNotBlank(resultCodeClassPackage)) {
                ...
                for (String pack : resultCodeClassPackages) {
                    Collections.addAll(resourceList,
                            resolver.getResources("classpath*:" + pack.replace(".", "/") + "/*.class"));
                }
            }
                    ...
                    for (ISubResultCode stateEnum : inter) {
                        ...
                        if (classify.equals(ResultCodeEnum.COMMON)) {
                            if (ifThisEnumFistRead) {
                                ...
                                sbFirst.append(
                                        "<tr><td colspan=\"2\" style=\"background-color: aliceblue;text-align: center;\">")
                                        .append(classify.getCodeName()).append("</td></tr>");
                            }
                            sbFirst.append("<tr><td>").append(stateEnum.getCode()).append("</td><td>")
                                    .append(stateEnum.getCodeMsg()).append("</td></tr>");
                        } else {
                            if (ifThisEnumFistRead) {
                                ifThisEnumFistRead = false;
                                sb.append(
                            ...
                            sb.append("<tr><td>").append(stateEnum.getCode()).append("</td><td>")
                                    .append(stateEnum.getCodeMsg()).append("</td></tr>");
                        }
                    }
                }
            ...
            sbAll.append(
                    "<table><thead><tr><th style=\"font-size:20px;\"><strong>状态码</strong></th><th style=\"font-size:20px;\"><strong>说明</strong></th></tr></thead>");
            sbAll.append("<tbody>");
